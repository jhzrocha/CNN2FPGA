...
class ParallelMultiplicator(ComponentCommonMethods):

    
    def __init__(self, inputQtBits = 8):
        self.minimalComponentFileName = f'parallel_multiplicator_{inputQtBits}b'
        self.portMap =   { 'in': [
                                Port('i_DATA',f'signed(0 to {inputQtBits-1})'),
                                Port('i_KERNEL',f'signed(0 to {inputQtBits-1})'),
        ...
        self.internalOperations = f"""
            ...
            port(a : in  std_logic_vector({inputQtBits-1} downto 0);  -- multiplicand
                b : in  std_logic_vector({inputQtBits-1} downto 0);  -- multiplier
                p : out std_logic_vector({inputQtBits+inputQtBits-1} downto 0)); -- product
            end pmul8;

            architecture circuits of pmul8 is
            constant N  : integer := {inputQtBits-1};     -- last row number
